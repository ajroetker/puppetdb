...
(defn annotate-command
  ...
  {:pre  [(map? message)]
   :post [(map? %)]}
  (assoc-in message [:annotations] {:received (or received (kitchensink/timestamp))
...
(defrecord Command [command version payload])
(defn-validated enqueue-command! :- s/Str
  ...
  (let [command-map (annotate-command (Command. command version payload))]
