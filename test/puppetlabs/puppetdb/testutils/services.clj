(ns puppetlabs.puppetdb.testutils.services
  (:require [puppetlabs.puppetdb.testutils :refer [temp-dir temp-file]]
            ...
            [puppetlabs.puppetdb.command.core :refer [command-service]]
   ...
   (let [[log-file config] (-> config
     ...
     (try
       (tkbs/with-app-with-config server
         (concat [jetty9-service svcs/puppetdb-service command/command-service
                  message-listener-service command-service webrouting-service metrics-service]
                 services)
         config
