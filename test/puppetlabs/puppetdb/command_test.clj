...
(def v5-catalog-command
  "Creates a small version 5 catalog command for testing"
...
(deftest replace-catalog
  (doverseq [version catalog-versions
             :let [command (munge-catalog-command version v5-catalog-command)]]
...
(deftest catalog-with-updated-resource-exported
  (doverseq [version catalog-versions
             :let [command (munge-catalog-command version {:command (command-names :replace-catalog) :version 5 :payload basic-wire-catalog})
                   command-1 (stringify-payload command)
...
(defn default-fact-munging
  "Converts the command to the `target-version`."
  [target-version]
  (case target-version
    identity))
...
(let [certname  "foo.example.com"
  ...
  (deftest replace-facts-no-facts
    (doverseq [version fact-versions
               :let [command (munge-fact-command version v3-command)]]
...
(deftest replace-facts-bad-payload
  (let [bad-command {:command (command-names :replace-facts)
                     ...
                     :payload "bad stuff"}]
    (doverseq [version fact-versions
               :let [command (munge-fact-command version bad-command)]]
...
(deftest concurrent-catalog-updates
  (testing "Should allow only one replace catalogs update for a given cert at a time"
    (let [test-catalog (get-in catalogs [:empty])
          wire-catalog (get-in wire-catalogs [5 :empty])
          nonwire-catalog (catalog/parse-catalog wire-catalog 5)
          certname     (get-in wire-catalog [:name])
...
(let [report (-> (:basic report-examples/reports)
                 (remove-environment :v3)
                 munge-example-report-for-storage)
      v3-command (assoc-in
                   {:command (command-names :store-report)
                    :version 3
                    :payload report}
  ...
  (deftest store-report
    (testing "should store the report"
      (doverseq [version report-versions
                 :let [command (munge-report-command version v3-command)]]
        (test-msg-handler command publish discard-dir
